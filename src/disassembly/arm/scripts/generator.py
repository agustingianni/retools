...
def create_decoders(filename):
    ...
    with open(filename, "w") as fd:
        ...
        for instruction in instructions:
            ...
            hard = ["cond", "coproc", "opc1", "CRd", "CRn", "CRm", "opc2", "option", "B"]
...
def create_to_string(filename):
    ...
    reg2string["Rdm"] = "regular_reg_str(ins->dm)"
    reg2string["Rdn"] = "regular_reg_str(ins->dn)"
    ...
    reg2string["imm"] = "itoa(ins->imm32)"
    reg2string["imm4"] = "itoa(ins->imm32)"
    reg2string["imm5"] = "itoa(ins->imm32)"
    reg2string["imm8"] = "itoa(ins->imm32)"
    reg2string["imm12"] = "itoa(ins->imm32)"
    reg2string["imm16"] = "itoa(ins->imm32)"
    reg2string["imm24"] = "itoa(ins->imm32)"
    reg2string["const"] = "itoa(ins->imm32)"
    reg2string["label"] = "itoa(ins->imm32)"
    
    reg2string["lsb"] = "itoa(ins->lsbit)"
    reg2string["width"] = "itoa(32 - ins->lsbit)"
    
    reg2string["type"] = "shift_type_str(ins->type)"
    reg2string["coproc"] = "coproc_str(ins->coproc)"
    reg2string["opc1"] = "itoa(ins->opc1)"
    reg2string["opc2"] = "itoa(ins->opc2)"
    ...
    reg2string["Qd"] = "quad_reg_str(ins->Qd)"
    reg2string["Qn"] = "quad_reg_str(ins->Qn)"
    reg2string["Qm"] = "quad_reg_str(ins->Qm)"

    reg2string["Dd"] = "double_reg_str(ins->Dd)"
    reg2string["Dn"] = "double_reg_str(ins->Dn)"
    reg2string["Dm"] = "double_reg_str(ins->Dm)"
    
    reg2string["Sd"] = "simple_reg_str(ins->Sd)"
    reg2string["Sn"] = "simple_reg_str(ins->Sn)"
    reg2string["Sm"] = "simple_reg_str(ins->Sm)"
        ...
        for instruction in instructions:
            ...
            for i, val in enumerate(op_args.name):
                if type(val) is ARMv7Parser.MandatoryToken:
                    ...
                    if val.name == "option":
                        print "ID:", instruction_id_name(instruction)
